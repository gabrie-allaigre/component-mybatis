...
public class AssociationSortResultFactory extends AbstractSortResultFactory<Association> {
    ...
    public AssociationSortResultFactory() {
    ...
    public SqlResult buildComponentSortResult(ISortResultContext sortResultContext, ComponentDescriptor<?> componentDescriptor, ComponentDescriptor.PropertyDescriptor propertyDescriptor,
        ...
        if (StringUtils.isNotBlank(joinTableName)) {
            return sortResultContext.visit(subComponentDescriptor, sortDirection, current, nextPropertyName, joinTableName, context);
        } else {
            return buildJoins(sortResultContext, componentDescriptor, propertyDescriptor, subComponentDescriptor, sortDirection, previousPropertyName, nextPropertyName, tableJoinName, context);
        }
    }

    private SqlResult buildJoins(ISortResultContext sortResultContext, ComponentDescriptor<?> componentDescriptor, ComponentDescriptor.PropertyDescriptor propertyDescriptor,
            ComponentDescriptor<?> subComponentDescriptor, SortDirection sortDirection, String previousPropertyName, String nextPropertyName, String tablePrefix, EngineContext context) {
        String current = (StringUtils.isNotBlank(previousPropertyName) ? previousPropertyName + "." : "") + propertyDescriptor.getPropertyName();
        ...
        SqlResult crr = sortResultContext.visit(subComponentDescriptor, sortDirection, current, nextPropertyName, joinTableName, context);
    ...
    private SqlResult.Join buildJoin(String tableName, String joinName, String previousJoinName, String[] sourceColumns, String[] targetColumns) {
        ...
        return SqlResult.Join.of(SqlResult.Join.Type.LeftOuter, tableName + " " + joinName + " ON " + sj.toString());
    }
}
